...
install_dependencies() {
	...
	apt install -y termux-tools coreutils less fontconfig-utils git zsh figlet toilet lf curl wget micro lsd man || {
		printf "${red}${LANG_STRINGS[1]}!${nocol}\n"
		exit 1
	}
}
...
configure_termux() {
	...
	if [ -d "${HOME}/.termux" ]; then
		...
		printf "${green}${LANG_STRINGS[3]}${nocol}: ${HOME}/.termux_bak_${_date_time}\n"
